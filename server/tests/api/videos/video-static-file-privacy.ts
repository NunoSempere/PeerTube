...
describe('Test video static file privacy', function () {
    ...
    function runSuite () {
      ...
      async function checkPrivateWebTorrentFiles (uuid: string) {
        ...
        for (const file of video.files) {
          ...
          expectStartWith(torrent.urlList[0], server.url + '/static/webseed/private/')

          const magnet = decode(file.magnetUri)
          expectStartWith(magnet.urlList[0], server.url + '/static/webseed/private/')

          await makeRawRequest({ url: file.fileUrl, token: server.accessToken, expectedStatus: HttpStatusCode.OK_200 })
          await makeRawRequest({ url: torrent.urlList[0], token: server.accessToken, expectedStatus: HttpStatusCode.OK_200 })
          await makeRawRequest({ url: magnet.urlList[0], token: server.accessToken, expectedStatus: HttpStatusCode.OK_200 })
        }
  ...
  describe('VOD static file right check', function () {
    ...
    async function checkVideoFiles (options: {
      ...
      const { id, expectedStatus, token } = options
    ...
    it('Should not be able to access a private video files without token', async function () {
      this.timeout(120000)

      const { uuid } = await server.videos.quickUpload({ name: 'video', privacy: VideoPrivacy.INTERNAL })
      await waitJobs([ server ])

      await checkVideoFiles({ id: uuid, expectedStatus: HttpStatusCode.FORBIDDEN_403, token: null })
    })

    it('Should not be able to access an internal video files without appropriate token', async function () {
      this.timeout(120000)

      const { uuid } = await server.videos.quickUpload({ name: 'video', privacy: VideoPrivacy.PRIVATE })
      await waitJobs([ server ])

      await checkVideoFiles({ id: uuid, expectedStatus: HttpStatusCode.FORBIDDEN_403, token: userToken })
    })

    it('Should be able to access a private video files with appropriate token', async function () {
      this.timeout(120000)

      const { uuid } = await server.videos.quickUpload({ name: 'video', privacy: VideoPrivacy.PRIVATE })
      await waitJobs([ server ])

      await checkVideoFiles({ id: uuid, expectedStatus: HttpStatusCode.OK_200, token: server.accessToken })
    })

    it('Should be able to access a private video of another user with an admin token', async function () {
      this.timeout(120000)
