...
async function moveFiles (options: {
  ...
  for (const file of video.VideoFiles) {
    ...
    try {
      ...
      logger.error(
        'Cannot move webtorrent file %s to %s after privacy change',
        directories.webtorrent.old, directories.webtorrent.new, { err }
      )
    }
  }
  ...
  if (hls) {
    ...
    try {
      ...
      logger.error(
        'Cannot move HLS file %s to %s after privacy change',
        directories.webtorrent.old, directories.webtorrent.new, { err }
      )
    }
  }
